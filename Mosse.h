...
{
    /*
     Element-wise division of complex numbers in src1 and src2 (in OpenCV C syntax)
     */
...
void Mosse::DefineGoal(void)
{
    ...
    GaussianBlur(g, g, Size(-1,-1), 2.0, 0);
...
double Mosse::Correlate(const Mat &image_sub, Point &delta_xy)
{
    ...
    //************************ This is where F*H  ************************
    mulSpectrums(IMAGE_SUB, H, RESPONSE, 0, true ); // multiplied by current H
...
bool Mosse::Run(const Mat &frame)
{
    ...
    while (1)
    {
        ...
        // correlate //Run is the only function that calls correlate() // In correlate, decide delta_xy
        Point delta_xy;
        PSR = Correlate(image_sub, delta_xy); //use H_i-1 (the last filter)
    ...
    A = A_new*rate + A*(1 - rate);
    B = B_new*rate + B*(1 - rate);
    UpdateFilter();
    return true;
